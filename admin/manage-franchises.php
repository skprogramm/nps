...
if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
    ...
    if ($search_field === 'all') {
        $where_clause = "WHERE (name LIKE ? OR email LIKE ? OR phone LIKE ? OR district LIKE ? OR state LIKE ?)";
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    fputcsv($output, ['Name', 'Email', 'Phone', 'Address', 'District', 'State', 'PIN', 'Submitted At']);

    // Build the export query with search filters
    $export_query = "SELECT * FROM franchises $where_clause ORDER BY submitted_at DESC";
    if (!empty($params)) {
    ...
    while ($row = $result->fetch_assoc()) {
        fputcsv($output, [$row['name'], $row['email'], $row['phone'], $row['address'], $row['district'], $row['state'], $row['pin'], $row['submitted_at']]);
    }
    fclose($output);
    exit;
}
...
// Fetch franchise records with search and pagination
$main_query = "SELECT * FROM franchises $where_clause ORDER BY submitted_at DESC LIMIT ? OFFSET ?";
if (!empty($params)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM franchises ORDER BY submitted_at DESC LIMIT ? OFFSET ?");
...
<head>
    ...
    <title>Manage Franchises | NPS Admin</title>
...
<body class="bg-gray-100">
    ...
    <main class="p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">📋 Manage Franchises</h1>
        ...
        <div class="mb-4">
            <p class="text-gray-600">
                <?php if (!empty($search_query)): ?>
                    ...
                    <?php endif; ?>
                <?php else: ?>
                    Showing all <?= $total_records ?> franchise application(s)
                <?php endif; ?>
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr class="border-b text-sm hover:bg-gray-50">
                            ...
                            <td class="p-3"><?= htmlspecialchars($row['pin']) ?></td>
